...
run_metropolis_MCMC_betas<-function(N,startvalue_betas1,startvalue_betas2,startvalue_betas3, gamma_prior,iterations,x111,x222,x333,x444,y,theta){
  ...
  for(i in 1:N){
   ...
   for (i in 1:N) {
    ...
    #######update the tau(extra zeros) parameter in ZIP######
	xcur=theta[iter]
	xpro=rbeta(1,length(y)/(length(y)/2),proshape)
	
	acceptratio=min(1,likelihood1(pred1,proposal1,z[[iter]],y,x111,x222,x333,x444,theta)*dbeta(xcur,5,2,log=FALSE)/(likelihood1(pred1,chain1[iter,],z[[iter]],y,x111,x222,x333,x444,theta)*dbeta(xpro,2,5,log=FALSE)))
    ...
    ############simulated data using the initial interaction parameter#############
     x_data = pz_123(z[[iter]],sum1,y,pred1,pred2,pred3,gamma_prior,chain1,chain2,chain3,x111,x222,x333,x444,theta,iterations)
     updated_data=pz_123(z[[iter]],sum1,y,pred1,pred2,pred3,chain,chain1,chain2,chain3,x111,x222,x333,x444,theta,iterations)
	
    dist_data1= abs(sum(y) - sum(x_data))  #difference between the original data and the simulated data.
    dist_data=as.vector(dist_data1)
    epsilon = round(quantile(dist_data,0.01))
    proposal = proposalfunction()
      if(dist_data<as.numeric(epsilon)){
