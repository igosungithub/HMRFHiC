...
likelihood1 <- function(pred1,param1,z,y,x111,x222,x333,x444,theta){
  y1=y[z==1]
  singlelikelihoods11 =  ifelse(y1==0,((theta+(1-theta))*(exp(-pred1(param1,z,x111,x222,x333,x444)))),(dpois(y1, lambda = exp(pred1(param1,z,x111,x222,x333,x444)), log = F)*(1-theta)))
...
prior1 <-function(param1,y,x111,x222,x333,x444,z){
  a1 = param1[1]
  b1 = param1[2]
  c1 = param1[3]
  d1 = param1[4]
  ...
  #  inversesdy=rgamma(1,(length(y1)-1)/2,(sum((y1-mean(y1))^2)/2))
  inversesdy=rgamma(1,(10-1)/2,100)
  sdy=1/inversesdy
  # meany1=rnorm(1,mean(y1),sdy/length(y1))
  meany1=rnorm(1,50,sd(y1)/2000)
  ...
  #cat("sd1=",sd1,",","sdy=",sdy,",","meany1=",meany1,",","mean1=",mean1,"\n")
  return(a1prior+b1prior+c1prior+d1prior+e1prior)
}

######Prior distribution for component 2######
prior2 <- function(param2,y,x111,x222,x333,x444,z){
  a2 = param2[1]
  b2 = param2[2]
  c2 = param2[3]
  d2 = param2[4]
  ...
  inversesdy2=rgamma(1,(200-1)/2,20000)
  sdy2=1/inversesdy2
  #meany2=rnorm(1,mean(y2),sdy2/length(y2))
  meany2=rnorm(1,300,sd(y2)/500)
...
prior3 <- function(param3,y,x111,x222,x333,x444,z){
  ...
  #inversesdy2=rgamma(1,(length(y2)-1)/2,(sum((y2-mean(y2))^2)/2))
  inversesdy3=rgamma(1,(50000000-1)/2,50000)
  sdy3=1/inversesdy3
  #meany2=rnorm(1,mean(y2),sdy2/length(y2))
  meany3=rnorm(1,500,sd(y3)/500)
  ...
  inversesdx2=rgamma(1,(length(x2)-1)/2,(sum((x2-mean(x2))^2)/2))
  sdx2=1/inversesdx2
  meanx2=rnorm(1,mean(x2),sd(x2)/length(x2))
...
########prior of theta in the ZIP likelihood##############
prior_theta<-function(theta){
	a=theta
	thetaprior<-dgamma(a,0.2,1,log=T)
	return(thetaprior)
}
...
posterior1 <- function(pred1,param1,z,y,x1,x2,x3,x4,theta){
  return (likelihood1(pred1,param1,z,y,x1,x2,x3,x4,theta) + prior1(param1,y,x111,x222,x333,x444,z))
}

#####Posterior distribution for component 2######
posterior2 <- function(pred2,param2,z,y,x1,x2,x3,x4){
  return (likelihood2(pred2,param2,z,y,x1,x2,x3,x4) + prior2(param2,y,x111,x222,x333,x444,z))
}
...
posterior3 <- function(pred3,param3,z,y,x1,x2,x3,x4){
  ...
  p3=prior3(param3,y,x111,x222,x333,x444,z)
  sumposterior3= l3 + p3
  #cat("l3=",l3,",","p3=",p3,"\n")
...
######posterior of theta in the ZIP likelihood###############
posterior_theta<-function(pred1,param1,z,y,x111,x222,x333,x444,theta){
	return(likelihood1(pred1,param1,z,y,x111,x222,x333,x444,theta) + prior_theta(theta))
}
...
############proposal function of theta in the ZIP likelihood#############
proposalfunctiontheta<-function(theta){
	return(rgamma(1,theta,2))
}
...
#############To calculate the Mixture model for the z update###########
pz_123=function(z,sum_neighbours,y,pred1,pred2,pred3,chain,chain1,chain2,chain3,x111,x222,x333,x444,theta, iterations){
  for (iter in 1:iterations) {
    y11=y[z==1]
    y22=y[z==2]
    y33=y[z==3]

    a=exp(chain[iter]*sum_neighbours[z==1])*ifelse(y11==0,(theta+(1-theta))*(exp(-pred1(chain1[iter,],z,x111,x222,x333,x444))),(dpois(y11,exp(pred1(chain1[iter,],z,x111,x222,x333,x444)),log=F)*(1-theta)))
    b=exp(chain[iter]*sum_neighbours[z==2])*dpois(y22,exp(pred2(chain2[iter,],z,x111,x222,x333,x444)),log=F)
    c=exp(chain[iter]*sum_neighbours[z==3])*dpois(y33,exp(pred3(chain3[iter,],z,x111,x222,x333,x444)),log=F)
...
proposaldensity1 <- function(param1){
  ...
  aprodensity1 = dnorm(a1,1, 0.1, log=T)
...
proposaldensity2 <- function(param2){
  ...
  aprodensity2 = dnorm(a2, 3,0.05, log=T)
  bprodensity2 = dnorm(b2, mean = 2, sd = 0.7, log=T)
  cprodensity2 = dnorm(c2, 0.4, 0.1, log = T)
  dprodensity2 = dnorm(d2, mean=0.5, sd=0.9, log = T)
...
proposaldensity3 <- function(param3){
  ...
  aprodensity3 = dgamma(a3,  70, 0.2, log=T)
  ...
  eprodensity3 = dnorm(e3, mean = 2, sd = 0.1, log = T)

  return(aprodensity3+bprodensity3+ cprodensity3 + dprodensity3 + eprodensity3)
}


########proposaldensity of theta in the ZIP likelihood##########
proposaldensitytheta<-function(theta){
	a=theta
	aprodensitytheta=dgamma(a,2,0.1,log=T)
	
	return(aprodensitytheta)
}
...
run_metropolis_MCMC_betas<-function(N,startvalue_betas1,startvalue_betas2,startvalue_betas3, gamma_prior,iterations,x111,x222,x333,x444,y,theta){
...
Pros2[i,j]=sample(1:3,1)
    ...
    P_initials=pz_123(z[[iter]],sum1,y,pred1,pred2,pred3,chain,chain1,chain2,chain3,x111,x222,x333,x444,theta,iterations) ##### full Conditional using Initial z
    P_proposed1=pz_123(Pros1,sum2,y,pred1,pred2,pred3,chain,chain1,chain2,chain3,x111,x222,x333,x444,theta,iterations) ##### full Conditional using first proposed z
    P_proposed2=pz_123(Pros2,sum3,y,pred1,pred2,pred3,chain,chain1,chain2,chain3,x111,x222,x333,x444,theta,iterations) ##### full Conditional using second proposed z
...
#    cat("z[[1]]=",z[[1]],"\n")
    ...
    a1=posterior1(pred1,proposal1,z[[iter]],y,x111,x222,x333,x444,theta)
    b1=posterior1(pred1,chain1[iter,],z[[iter]],y,x111,x222,x333,x444,theta)
    a2=posterior2(pred2,proposal2,z[[iter]],y,x111,x222,x333,x444)
    b2=posterior2(pred2,chain2[iter,],z[[iter]],y,x111,x222,x333,x444)
    a3=posterior3(pred3,proposal3,z[[iter]],y,x111,x222,x333,x444)
    b3=posterior3(pred3,chain3[iter,],z[[iter]],y,x111,x222,x333,x444)
...
proposaltheta = proposalfunctiontheta(chaintheta[iter])
ta=posterior_theta(pred1,chain1[iter,],z[[iter]],y,x111,x222,x333,x444,proposaltheta)
probabtheta1 = (ta-posterior_theta(pred1,chain1[iter,],z[[iter]],y,x111,x222,x333,x444,chaintheta[iter])-proposaldensitytheta(chaintheta[iter])+proposaldensitytheta(proposaltheta))
	
 if(log(runif(1)) < probabtheta1){
		theta[iter+1]<- proposaltheta
	 }else{
